...
for marker in ('N.O.M.A.D. CCTV Digital Twin Simulator 8e.7.5','id="appVersionLabel"','id="openVideoWall"','id="popOutVideoWall"','id="videoWallOverlay"','id="videoWallGrid"','video-wall-empty-slot','--wall-rows','id="helpGroup"','id="aboutGroup"','All rights reserved.','/static/viewer.js?v='):
    assert marker in HTML, f'Missing 8e.7 UI marker: {marker}'
for marker in ("const APP_VERSION = '8e.7.5';","const MAX_CAMERA_VIEWPORTS = 16;","function focusCameraViewport(viewport)","function buildIntegratedVideoWall()","function buildPopupVideoWall()","function getVideoWallLayout(count, selection)","function appendVideoWallEmptySlots(doc, grid, count)","function reorderVideoWall(draggedKey, targetKey)","key: 'scene', label: 'Planning Scene'","viewport.addEventListener('mousedown', () => focusCameraViewport(viewport), true)","isRenderable: () => !isMinimized","openCameraViewports[index].renderer.dispose()","wallRenderer.forceContextLoss?.()","openCameraViewports[index].renderer.forceContextLoss?.()"):
