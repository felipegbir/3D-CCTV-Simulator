...
Cache 956 deployment passed on CCTV PID 4142049 at port 5010 with health status ok, exact local/deployed frontend SHA-256 hashes, and live cache-956 marker. NAS baseline: NOMAD_8e.7.1_CACHE955_PRE_956_20260727_161726; server rollback: deploy_backups/pre_cache_956_20260727_1645. Only verified CCTV PID 4117245 was replaced; independent processes/containers were untouched. Owner QA pending.

### 8e.7.1 cache-957 consolidated backlog release - deployment pending

Cache 957 preserves camera sensor aspect under sidebar, viewport, Video Wall, and preset-panel resizing and aligns normalized ROI overlays to the fitted canvas. ROI overlays now appear only for the actively recalled parent preset and hide immediately on manual camera/PTZ/depth changes. Automatic preset naming increments per camera. PTZ/ROI workflow status no longer enters Measurement Tools. Configured camera replacement is guarded with Cancel, Replace-and-Clear, and Clone-and-Apply workflows; unconfigured cameras change directly. The Planning Workspace adds a synchronized six-face Navigation Cube and a fixed-size dynamic engineering Scale Indicator, both saved preferences. Static audit, syntax, full Python regression, runtime modules, and cache-957 guards pass. Owner visual interaction QA remains pending.
Cache 957 deployment passed on CCTV PID 4182721 at port 5010 with health status ok, exact local/deployed frontend SHA-256 hashes, and live cache-957 marker. NAS baseline: NOMAD_8e.7.1_CACHE956_PRE_957_20260727; server rollback: deploy_backups/pre_cache_957_20260727. Only verified CCTV PID 4142049 was replaced; independent processes and containers were untouched. Owner visual interaction QA remains pending.

### 8e.7.1 cache-958 immutable sensor frame and corrective QA release - deployment pending

Cache 958 removes layout-driven camera reprojection: Camera View and Video Wall sensor render panes keep their saved sensor aspect and optical frame while maximize/restore, sidebar visibility, Presets panels, tile layouts, and browser window size change. Preset panels overlay the immutable render surface rather than resizing it. Main planning rendering now uses ResizeObserver plus window/visual-viewport settling so restore/maximize changes cannot leave a stale renderer size.

The Navigation Cube vertical mapping and drag direction are corrected; each face has a direct snap handler and scene-orbit animation ownership. PTZ preset recall hides child ROIs before motion, restores the saved camera root transform, PTZ, optics, and depth, and shows ROIs only after animation completion and renderer settling. Report preset captures composite saved ROI polygons and labels over their camera images. Configured-camera warnings now require actual saved analysis/configuration, suppress same-model/no-configuration prompts, and the Clone-and-Apply path resolves the newly created camera by object identity. Static audit, JavaScript syntax, full Python regression, and a dedicated cache-958 guard pass. Owner visual interaction QA remains pending.Cache 958 deployment passed on CCTV PID 17055 at port 5010 with HTTP 200 on the application root, exact local/deployed frontend SHA-256 hashes, live cache-958/report-ROI/immutable-width markers, and verified pinned SSH host key. NAS baseline: `NOMAD_8e.7.1_CACHE957_PRE_958_20260727_174203`; server rollback: `deploy_backups/pre_cache_958_20260727_1742`. Only verified CCTV PID 4182721 was replaced; independent PIDs 1458, 2912, and 2733584 remained running. This build has no `/health` route, so application-root HTTP 200 plus listener/process/hash checks were used. Owner visual interaction QA remains pending.
### 8e.7.1 cache-959 attached Presets dock and Video Wall recovery - deployment pending

Cache 959 corrects the cache-958 UI ownership regression. The Camera View sensor pane keeps a sensor-aspect outer frame and remains flex-centered; the Presets manager is positioned at the Camera View right edge as an attached sibling dock and consumes no sensor-image area. Maximized Camera Views derive height from sensor aspect instead of independently scaling width and height with sidebar state. The Video Wall tile is again the positioned containing block and its render pane is an in-flow nonzero flex host, restoring camera sources while Presets overlays remain non-destructive. The Scene Tree inner panel border, radius, and shadow are removed to mirror the Inspector rail structure. Full automated regression and owner visual QA remain pending.Cache 959 deployment passed on CCTV PID 106684 at port 5010 with application-root HTTP 200, exact local/deployed hashes, and live attached-dock/Video-Wall-host markers. NAS baseline: `NOMAD_8e.7.1_CACHE958_PRE_959_20260727`; server rollback: `deploy_backups/pre_cache_959_20260727`. Only verified CCTV PID 17055 was replaced; independent PIDs 1458, 2912, and 2733584 remained running. In-app browser automation was unavailable because of the Windows process-launch restriction; owner visual interaction QA is required.
### 8e.7.1 cache-960 permanent Camera View frame ratio and Video Wall right dock - deployment pending

Cache 960 defines the Camera View outer-window contract explicitly: normal, maximized, restored, and unminimized frames all retain the original 320 x 240 (4:3) frame ratio, independent of left/right rail state and independent of the sensor canvas aspect fitted inside. The 1 x 1 Video Wall vertical Presets dock now clears the general `left: 0` anchor with `left: auto` and anchors exclusively at `right: 0`; this removes the left-side panel placement and the reserved right-side ghost area. Automated static, syntax, complete regression, and dedicated cache-960 guards are required before deployment.Cache 960 deployment passed on CCTV PID 139261 at port 5010 with HTTP 200, exact local/deployed hashes, exactly two 4:3 frame-contract markers, and the exclusive right-dock marker. NAS baseline: `NOMAD_8e.7.1_CACHE959_PRE_960_20260727`; server rollback: `deploy_backups/pre_cache_960_20260727`. Only verified CCTV PID 106684 was replaced; independent PIDs 1458, 2912, and 2733584 remained running. Owner visual confirmation remains pending.
### 8e.7.2 baseline promotion and HTML user guide — deployment pending

The owner accepted cache 960 and closed 8e.7.1. Version 8e.7.2 promotes that stabilized feature set, corrects the attached Camera View Presets dock to border-box sizing so its external height exactly equals the owning Camera View, and publishes a responsive dark-themed HTML user guide from Help. The guide covers project files, workspace navigation, models/references, cameras, PTZ presets, depth surfaces, polygon ROIs, pixel-density analysis, measurements, Video Wall, reports, preferences, shortcuts, and final QA. Lay Face Flat remains explicitly experimental. Full static, syntax, regression, runtime, link, and release guards are required before deployment.