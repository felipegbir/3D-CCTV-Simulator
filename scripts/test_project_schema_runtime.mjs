...
alerts.length = 0;
assert.equal(context.validateProjectSchema({ schemaVersion: 3 }), true);
assert.equal(alerts.length, 0);

assert.equal(context.validateProjectSchema({}), true);
assert.match(alerts.pop(), /Legacy project schema 1/);

assert.equal(context.validateProjectSchema({ schemaVersion: 4 }), false);
...
console.log('8e.7 runtime schema and asset manifest tests passed');
