"""8e.7.5 camera identity, optical geometry, and report-view regression guard."""
...
assert "const APP_VERSION = '8e.7.5';" in JS
for marker in (
    "N.O.M.A.D. CCTV Digital Twin Simulator 8e.7.5",
    'id="appVersionLabel">8e.7.5',
    'id="menuVersionLabel">8e.7.5',
    "/static/viewer.js?v=875",
):
    assert marker in HTML, marker
assert "Version 8e.7.5" in GUIDE

create_start = JS.index("function createCameraObject")
create_end = JS.index("// Load the bundled starter model", create_start)
...
load_start = JS.index("async function applyLoadedProject(project)")
...
print("8e.7.5 camera identity, optical geometry, and report-view guard passed")